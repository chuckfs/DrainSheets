...
- [x] ✅ **Shareable links + Viewer access** — DONE (Jun 20). _Viewer (read-only) and Commenter sharing already existed_ — you can grant someone view-only access from the Share dialog, and the grid enforces it. **New:** the Share dialog now has a **"Share via link"** section — create one link per sheet that grants Viewer / Commenter / Editor access to anyone in your org who opens it. Copy the link, change its role, enable/disable it, or regenerate it (which instantly kills the old link). Opening a link grants access and drops you on the sheet; it never downgrades access you already have. _Security: links only work for people logged into the same organization (no anonymous public access in this version), and can never grant admin/owner. Adds a new database table → needs `npm run db:reset` + `npm run db:types` before testing._
- [x] ✅ **Pagination on long notes/document lists** — DONE (Jun 20). Notes and attachment lists now load 25 at a time with a **"Load more"** button, instead of pulling everything at once — so a row or sheet with hundreds of notes/files stays fast. _No database change, so this one works on a refresh (no `db:reset` needed)._
